...
public OnPluginStart() {
	RegAdminCmd("+copyent", Command_Copy, 0, "Copy a prop.");
...
public Action:Command_Copy(Client, args) {
	...
	if (g_bCopyIsRunning[Client]) {
		if (g_bClientLang[Client])
			LM_PrintToChat(Client, "你正在複製其他物件!");
		else
			LM_PrintToChat(Client, "You are already copying something!");
		return Plugin_Handled;
	}
	...
	if (StrEqual(szClass, "prop_ragdoll") || StrEqual(szClass, "player")) {
		if (LM_IsAdmin(Client, true)) {
			...
			if (g_bClientLang[Client])
				LM_PrintToChat(Client, "你需要 \x04二級建造權限\x01 才能複製此物件!");
			else
				LM_PrintToChat(Client, "You need \x04L2 Build Access\x01 to copy this prop!");
			return Plugin_Handled;
		}
	} else {
		if (StrEqual(szClass, "func_physbox") && !LM_IsAdmin(Client, true)) {
			if (g_bClientLang[Client])
				LM_PrintToChat(Client, "你無法複製此物件!");
			else
				LM_PrintToChat(Client, "You cant copy this prop!");
			return Plugin_Handled;
		}
	...
	if (LM_SetEntityOwner(g_iCopyTarget[Client], Client, IsDoll)) {
		if (bCanCopy) {
			...
			if (StrEqual(szModelName, "models/props_c17/oildrum001_explosive.mdl") && !LM_IsAdmin(Client, true)) {
				if (g_bClientLang[Client])
					LM_PrintToChat(Client, "你需要 \x04二級建造權限\x01 才能複製此物件!");
				else
					LM_PrintToChat(Client, "You need \x04L2 Build Access\x01 to copy this prop!");
			...
			if (g_bClientLang[Client])
				LM_PrintToChat(Client, "此物件無法複製.");
			else
				LM_PrintToChat(Client, "This prop was not copy able.");
			return Plugin_Handled;
		}
	} else {
		RemoveEdict(g_iCopyTarget[Client]);
