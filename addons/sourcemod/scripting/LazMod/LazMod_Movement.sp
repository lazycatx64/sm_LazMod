...
public Action Command_EnableSprint(Client, args)
{
	...
	entSpeedmod = CreateEntityByName("player_speedmod");
...
TeleportPlayer(int ClientTo, int ClientSent)
{
	float fOriginTo[3], fAimTo[3];
	GetClientAbsOrigin(Client, fOriginTo);
	GetClientEyeAngles(Client, fAimTo);
	fOriginTo[2] += 75;

	TeleportEntity(ClientSent, fOriginTo, fAimTo);
}