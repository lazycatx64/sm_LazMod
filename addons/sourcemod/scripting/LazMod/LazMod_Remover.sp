...
public OnPluginStart() {	
	...
	RegAdminCmd("sm_da", Command_DeleteAll, 0, "Delete all of your spawned entitys.");
	RegAdminCmd("sm_del", Command_Delete, 0, "Delete an entity.");
	RegAdminCmd("sm_d", Command_Delete, 0, "Delete an entity.");
...
public OnMapStart() {
	...
	for (new i = 1; i < MaxClients; i++) {
		...
		if (LM_IsClientValid(i, i))
			GetClientAuthString(i, g_szConnectedClient[i], sizeof(g_szConnectedClient));
	}
}

public OnClientPutInServer(Client) {
	GetClientAuthString(Client, g_szConnectedClient[Client], sizeof(g_szConnectedClient));
}

public OnClientDisconnect(Client) {
	g_szConnectedClient[Client] = "";
	GetClientAuthString(Client, g_szDisconnectClient[Client], sizeof(g_szDisconnectClient));
	new iCount;
	...
	LM_SetLimit(Client, 0);
	LM_SetLimit(Client, 0, true);
	if (iCount > 0) {
		new Handle:hPack;
...
public Action:Timer_Disconnect(Handle:Timer, Handle:hPack) {
	...
	new iTime = ReadPackCell(hPack);
	if (iTime < g_iDisconnectTime) {
		for (new iClient = 1; iClient < sizeof(g_szConnectedClient); iClient++) {
			if (StrEqual(g_szConnectedClient[iClient], g_szDisconnectClient[Client])) {
				new iCount = 0, String:szClass[32];
				for (new iEnt = 0; iEnt < MAX_HOOK_ENTITIES; iEnt++) {
					if (IsValidEdict(iEnt)) {
						if (g_iTempOwner[iEnt] == iClient) {
							GetEdictClassname(iEnt, szClass, sizeof(szClass));
							if (StrEqual(szClass, "prop_ragdoll"))
								LM_SetEntityOwner(iEnt, iClient, true);
							else
								LM_SetEntityOwner(iEnt, iClient);
							iCount++;
							g_iTempOwner[iEnt] = -1;
						}
					}
				...
				if (g_bClientLang[Client])
					LM_PrintToChat(Client, "你取回了 %i 個物件.", iCount);
				else
					LM_PrintToChat(Client, "You got back %i prop(s).", iCount);
				return;
			}
		}
		iTime++;
...
public Action:Command_DeleteAll(Client, args) {
	...
	if (iCount > 0) {
		if (g_bClientLang[Client])
			LM_PrintToChat(Client, "刪除了所有你擁有的物件.");
		else
			LM_PrintToChat(Client, "Deleted all props you owns.");
	} else {
		if (g_bClientLang[Client])
			LM_PrintToChat(Client, "你沒有任何物件.");
		else
			LM_PrintToChat(Client, "You don't have any props.");
	}
	
	LM_SetLimit(Client, 0);
	LM_SetLimit(Client, 0, true);
	FakeClientCommand(Client, "e_removespawned");
	...
	LM_Logging(Client, "sm_delall", szArgs);
	return Plugin_Handled;
}
...
public Action:Command_Delete(Client, args) {
	...
	new iEntity = LM_ClientAimEntity(Client, true, true);
	if (iEntity == -1) 
		return Plugin_Handled;
	...
	if (LM_IsEntityOwner(Client, iEntity)) {
		...
		if (!LM_IsAdmin(Client)) {
			if (StrEqual(szClass, "prop_vehicle_driveable") || StrEqual(szClass, "prop_vehicle") || StrEqual(szClass, "prop_vehicle_airboat") || StrEqual(szClass, "prop_vehicle_prisoner_pod")) {
				if (g_bClientLang[Client])
					LM_PrintToChat(Client, "你無法刪除此物件!");
				else
					LM_PrintToChat(Client, "You can't delete this prop!");
				return Plugin_Handled;
			}
		}
		...
		if (random == 1) {
			EmitAmbientSound("weapons/airboat/airboat_gun_lastshot1.wav", vOriginAim, iEntity, SNDLEVEL_NORMAL, SND_NOFLAGS, 0.5, iPitch);
			EmitAmbientSound("weapons/airboat/airboat_gun_lastshot1.wav", vOriginPlayer, Client, SNDLEVEL_NORMAL, SND_NOFLAGS, 0.5, iPitch);
		} else {
			EmitAmbientSound("weapons/airboat/airboat_gun_lastshot2.wav", vOriginAim, iEntity, SNDLEVEL_NORMAL, SND_NOFLAGS, 0.5, iPitch);
		...
		TE_SendToAll();
		TE_SetupBeamPoints(vOriginAim, vOriginPlayer, g_PBeam, g_Halo, 0, 66, 1.0, 3.0, 3.0, 0, 0.0, ColorRed, 20);
		TE_SendToAll();
		...
		if(StrEqual(szClass, "prop_ragdoll"))
			LM_SetLimit(Client, -1, true);
		else
			LM_SetLimit(Client, -1);
		LM_SetEntityOwner(iEntity, -1);
	}
