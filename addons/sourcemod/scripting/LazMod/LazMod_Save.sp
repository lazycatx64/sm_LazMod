...
#define DEBUG 0
#define MSGTAG "\x04SaveSystem\x01:"
...
public Plugin:myinfo = {
	...
	author = "LaZy cAt",
	description = "Save the craps which you built.",
	version = LAZMOD_VER,
	url = ""
};
...
public Action:Command_SaveSystem(Client, args) {
	if (!g_bListExist) {
		...
		if (g_bClientLang[Client]) {
			LM_PrintToChat(Client, "錯誤: 0x00005E");
			LM_PrintToChat(Client, "請通知管理員解決問題");
		} else {
			LM_PrintToChat(Client, "Error: 0x00005E");
			LM_PrintToChat(Client, "Please notify the admin");
		}
	...
	if (g_bIsRunning[Client]) {
		if (g_bClientLang[Client])
			LM_PrintToChat(Client, "%s 程序已經在執行中. 請稍後...", MSGTAG);
		else
			LM_PrintToChat(Client, "%s Process is already running. Please Wait...", MSGTAG);
		return Plugin_Handled;
	}
	...
	if ((StrEqual(szMode, "save") || StrEqual(szMode, "load") || StrEqual(szMode, "delete")) && args > 1) {
		...
		if (StrEqual(szMode, "save")) {
			if (FileExists(g_szFileName[Client])) {
				if (g_bClientLang[Client])
					LM_PrintToChat(Client, "%s 該存檔已存在. 取代中...", MSGTAG);
				else
					LM_PrintToChat(Client, "%s The save already exists. Replacing old save...", MSGTAG);
					
				if (!DeleteFile(g_szFileName[Client])) {
					if (g_bClientLang[Client])
						LM_PrintToChat(Client, "%s 取代檔案失敗! 存檔程序放棄!", MSGTAG);
					else
						LM_PrintToChat(Client, "%s Replace failed! Save process abort!", MSGTAG);
					return Plugin_Handled;
				}
			}
			if (g_bClientLang[Client])
				LM_PrintToChat(Client, "%s 開始儲存. 請稍後...", MSGTAG);
			else
				LM_PrintToChat(Client, "%s Saving the content. Please Wait...", MSGTAG);
			...
			if (!FileExists(g_szFileName[Client])) {
				if (g_bClientLang[Client])
					LM_PrintToChat(Client, "%s 該存檔不存在.", MSGTAG);
				else
					LM_PrintToChat(Client, "%s The save does not exist.", MSGTAG);
			} else {
				if (g_bClientLang[Client])
					LM_PrintToChat(Client, "%s 開始讀取. 請稍後...", MSGTAG);
				else
					LM_PrintToChat(Client, "%s Loading the content. Please Wait...", MSGTAG);
				g_bIsRunning[Client] = true;
			...
			if (FileExists(g_szFileName[Client])) {
				if (DeleteFile(g_szFileName[Client])) {
					if (g_bClientLang[Client])
						LM_PrintToChat(Client, "%s 存檔刪除成功.", MSGTAG);
					else
						LM_PrintToChat(Client, "%s Delete save successfully.", MSGTAG);
					CheckSaveList(Client, szMode, szSaveName, szSteamID);
				} else {
					if (g_bClientLang[Client])
						LM_PrintToChat(Client, "%s 存檔刪除失敗!", MSGTAG);
					else
						LM_PrintToChat(Client, "%s Delete save failed!", MSGTAG);
				}
			} else {
				if (g_bClientLang[Client])
					LM_PrintToChat(Client, "%s 該存檔不存在.", MSGTAG);
	...
	if (g_bClientLang[Client]) {
		LM_PrintToChat(Client ,"%s 用法:", MSGTAG);
		LM_PrintToChat(Client ,"%s !ss save <存檔名稱>   = 存檔", MSGTAG);
		LM_PrintToChat(Client ,"%s !ss load <存檔名稱>   = 讀取", MSGTAG);
		LM_PrintToChat(Client ,"%s !ss delete <存檔名稱> = 刪除", MSGTAG);
		LM_PrintToChat(Client ,"%s !ss list  = 列出你的存檔清單", MSGTAG);
	} else {
		LM_PrintToChat(Client ,"%s Usage:", MSGTAG);
		LM_PrintToChat(Client ,"%s !ss save <SaveName>", MSGTAG);
		LM_PrintToChat(Client ,"%s !ss load <SaveName>", MSGTAG);
		LM_PrintToChat(Client ,"%s !ss delete <SaveName>", MSGTAG);
		LM_PrintToChat(Client ,"%s !ss list", MSGTAG);
	}
	return Plugin_Handled;
}
...
public Action:Timer_Save(Handle:Timer, Handle:hDataPack) {
	...
	if (g_hFile[Client] == INVALID_HANDLE) {
		...
		if (g_iTryCount[Client] < 3) {
			...
			if (g_bClientLang[Client])
				LM_PrintToChat(Client, "%s 無法建立存檔! 程序放棄!", MSGTAG);
			else
				LM_PrintToChat(Client, "%s Unable to create the save! Process abort!", MSGTAG);
			g_hFile[Client] = INVALID_HANDLE;
			g_iTryCount[Client] = 0;
		}
	} else {
		...
		for (new i = 0; i < MAX_HOOK_ENTITIES; i++) {
			if (IsValidEdict(i)) {
				...
				if ((StrContains(szClass, "prop_dynamic") >= 0 || StrContains(szClass, "prop_physics") >= 0) && !StrEqual(szClass, "prop_ragdoll") && LM_GetEntityOwner(i) == Client) {
		...
		if (g_iCount[Client] > 0) {
			if (g_bClientLang[Client])
				LM_PrintToChat(Client, "%s 儲存了 %i 個物件", MSGTAG, g_iCount[Client]);
			else
				LM_PrintToChat(Client, "%s Saved %i prop(s)", MSGTAG, g_iCount[Client]);
			g_iCount[Client] = 0;
			CheckSaveList(Client, szMode, szSaveName, szSteamID);
		} else {
			if (g_bClientLang[Client])
				LM_PrintToChat(Client, "%s 未儲存任何物件.", MSGTAG);
			else
				LM_PrintToChat(Client, "%s No prop has been saved.", MSGTAG);
...
public Action:Timer_Load(Handle:Timer, Handle:hDataPack) {
	...
	if (g_hFile[Client] == INVALID_HANDLE) {
		...
		if (g_iTryCount[Client] < 3) {
			...
			if (g_bClientLang[Client])
				LM_PrintToChat(Client, "%s 無法讀取存檔! 程序放棄!", MSGTAG);
			else
				LM_PrintToChat(Client, "%s Unable to read the save! Process abort!", MSGTAG);
			g_hFile[Client] = INVALID_HANDLE;
			g_iTryCount[Client] = 0;
			return;
		}
		...
		if (ReadFileLine(g_hFile[Client], szLoadString, sizeof(szLoadString))) {
			if (StrContains(szLoadString, "ent") != -1) {
				...
				if (Obj_LoadEntity != -1) {
					if (LM_SetEntityOwner(Obj_LoadEntity, Client)) {
	...
	if (!IsEndOfFile(g_hFile[Client]) && !bRegOwnerError) {
		...
		g_bIsRunning[Client] = false;
		if (g_bClientLang[Client])
			LM_PrintToChat(Client, "%s 讀取了 %i 個物件, %i 個物件讀取失敗", MSGTAG, g_iCount[Client], g_iError[Client]);
		else
			LM_PrintToChat(Client, "%s Loaded %i props, failed to load %i props", MSGTAG, g_iCount[Client], g_iError[Client]);
		g_iCount[Client] = 0;
		g_iError[Client] = 0;
	}
	return;
...
public Action:Timer_List(Handle:Timer, Handle:hDataPack) {
	...
	if (g_hFile[Client] == INVALID_HANDLE) {
		...
		if (g_iTryCount[Client] < 3) {
			...
			if (g_bClientLang[Client])
				LM_PrintToChat(Client, "%s 無法列出清單! 程序放棄!", MSGTAG);
			else
				LM_PrintToChat(Client, "%s Unable to list the save! Process abort!", MSGTAG);
			g_hFile[Client] = INVALID_HANDLE;
			g_iTryCount[Client] = 0;
			return;
		}
		...
		if (g_iCount[Client] == 0) {
			if (g_bClientLang[Client])
				LM_PrintToChat(Client, "%s 你沒有任何存檔.", MSGTAG);
			else
				LM_PrintToChat(Client, "%s You don't have any save.", MSGTAG);
		} else {
			if (g_bClientLang[Client])
				LM_PrintToChat(Client, "%s 你總共有 %i 個存檔.", MSGTAG, g_iCount[Client]);
			else
				LM_PrintToChat(Client, "%s You have %i save(s).", MSGTAG, g_iCount[Client]);
			g_iCount[Client] = 0;
		}
	}
	return;
...
stock Save_CheckSaveName(Client, String:Check[32]) {
	if (strlen(Check) > 32) {
		if (g_bClientLang[Client])
			LM_PrintToChat(Client, "%s 存檔名字數上限是 32 個英文或數字不含中文及特殊符號!", MSGTAG);
		else
			LM_PrintToChat(Client, "%s The max SaveName length is 32 and does not allow symbols!", MSGTAG);
		return false;
	}
	...
	for (new i = 0; i < sizeof(Symbol); i++) {
		if (FindCharInString(Check, Symbol[i]) != -1) {
			if (g_bClientLang[Client])
				LM_PrintToChat(Client, "%s 存檔名不可含有特殊符號!", MSGTAG);
			else
				LM_PrintToChat(Client, "%s Symbols is not allowed in SaveName!", MSGTAG);
			return false;
		}
	}
	return true;
